...
def Sigmoid(x):
    ...
    slx=1/(1+np.exp(x))
    return slx
def gradient_logistics(x,y,w):
    gdlr=np.zeros(w.shape[0])
    for i in range(len(x)):
        lx=y[i]*np.dot(x[i],w)
        sig_lx=Sigmoid(lx)
        gdlr=gdlr+(y[i]*(1-sig_lx)*x[i])
    return gdlr



#training from data to find optimal weight vector
def logistics_classifier(training_X,training_y,learning_rate):
    mistake_count=0
    ...
    while(True):
        ...
        if h!=training_y[i]:
            gdmce=gradient_logistics(training_X,training_y,w)
            w+=learning_rate*gradient_logistics(training_X,training_y,w)
...
#call for training
weight_vector,mistakeCount=logistics_classifier(X_train,y_train,0.01)
#display results
